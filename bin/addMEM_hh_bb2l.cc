...
//--- declare missing transverse energy
  ...
  for ( const std::string & central_or_shift: central_or_shifts_hme )
    {
      const std::string branchName_hmeOutput_cos(Form("%s_%s", branchName_hmeOutput.data(), central_or_shift.data()));
      ...
      const std::string branchName_hmeOutput_missingBJet_cos(Form("%s_%s", branchName_hmeOutput_missingBJet.data(), central_or_shift.data()));
...
//--- therefore, we have to write the reco objects before re-reading new stuff
    ...
    if ( maxPermutations_addMEM_hh_bb2l >= 1 )
    {
	  ...
	  if ( method_MEM ) 
	  {
		...
		std::vector<RecoJet> jets_ak4_mem = jetReaderAK4->read();
		std::vector<const RecoJet*> jet_ptrs_ak4_mem = convert_to_ptrs(jets_ak4_mem);
		std::vector<const RecoJet*> cleanedJetsAK4_wrtLeptons = jetCleanerAK4_dR04(jet_ptrs_ak4_mem, fakeableLeptons);
		std::vector<RecoJetAK8> jets_ak8_mem = jetReaderAK8->read();
		std::vector<const RecoJetAK8*> jet_ptrs_ak8_mem = convert_to_ptrs(jets_ak8_mem);
		std::vector<const RecoJetAK8*> cleanedJetsAK8_wrtLeptons = jetCleanerAK8_dR08(jet_ptrs_ak8_mem, fakeableLeptons);
	  ...
	  if ( method_HME)
	  {
		...
		std::vector<RecoJet> jets_ak4_hme = jetReaderAK4->read();
		std::vector<const RecoJet*> jet_ptrs_ak4_hme = convert_to_ptrs(jets_ak4_hme);
		std::vector<const RecoJet*> cleanedJetsAK4_wrtLeptons = jetCleanerAK4_dR04(jet_ptrs_ak4_hme, fakeableLeptons);
		std::vector<RecoJetAK8> jets_ak8_hme = jetReaderAK8->read();
		std::vector<const RecoJetAK8*> jet_ptrs_ak8_hme = convert_to_ptrs(jets_ak8_hme);
		std::vector<const RecoJetAK8*> cleanedJetsAK8_wrtLeptons = jetCleanerAK8_dR08(jet_ptrs_ak8_hme, fakeableLeptons);
		std::vector<const RecoJetAK8*> selJetsAK8_Hbb = jetSelectorAK8_Hbb(cleanedJetsAK8_wrtLeptons, isHigherCSV_ak8);
		std::vector<const RecoJet*> selJetsAK4_Hbb = jetSelectorAK4(cleanedJetsAK4_wrtLeptons, isHigherCSV);
                //-------------------------------------------------------------------                                                                                                                       
                // select the two jets from the H->bb decay from either the AK4 (resolved H->bb) or AK8 (boosted H->bb) collection                                                                          
